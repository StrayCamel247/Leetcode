...
- 应用层
  - http，smtp，ftp，telnet等
  - 客户端web发送请求，根据请求的协议不同而进行不同的处理
- 表示层
  - ASCII，[ssl/tls](https://zhuanlan.zhihu.com/p/133375078)
  - 对请求返回的数据，是解析成图片还是文字，数据的压缩，加密打包，都是在这层来完成
- 会话层
  - adsp, rpc   
  - 负责建立和断开通信；可看作是快递的调度员；对数据是进行分包发送还是一次性发送，都是这层负责的
- 传输层
  - tcp/udp
  - 分则传输的可靠性；只在通信双方处理，而不是路由器上处理
  - tcp通过三次握手提供可靠的传输-丢包了可以重发，而udp不会-只会将数据发送到指定ip的端口
  - 指定通信端口-根据指定的端口而确定应用层所使用的协议
- 网络层
  - ip/icmp协议
  - 网络层是通过ip地址将数据从发送端主机发送到接收端主机
  - 从主机之间的多条网络链路中找到最顺畅的通道进行发送
- 数据链路层
  - hdlc，ppp，slip
  - 物理层面上互连设备之间的通信；比如一个以太网下不同设备的通信
  - 数据链路会将数据划分成有意义的数据帧发送给对方
  > 举个例子可能会更好理解，暂且把需要传输的数据看作为不同来源的水，如果直接倒入池子中时，是无法重新分辨出不同来源的水的。但如果将不同来源的灌入瓶子中并打上记号，那就能区分出不同来源的水。这也就是为什么要划分为具有意义的数据帧传送给对端。同时要注意的是，数据链路层只负责将数据运送给物理相连的两端，并不负责直接发送到最终地址。 数据链路层可以看作是快递公司的司机，他们驾驶着汽车，将打包好的货物（数据帧）从一个城市（物理节点）运输到另一个城市。
- 物理层
